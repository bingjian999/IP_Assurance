...
namespace bKwVJaKHQ9PJpSGiqkF;

internal static class GwPwjPKig3SC80BpOVZ
{
	[CompilerGenerated]
	...
	{
		Microsoft.Office.Interop.Excel.Application application = xcAKcictyYcoMl6ObA.gM4XBr1Rq();
		if (application == null)
		{
			F2ZFeLcsiLlLr89kqUl.F9Ycoqv2I8("无法启动 Excel/WPS 表格。", "IP_Assurance");
			return;
		}
		...
		if (tables.Count == 0)
		{
			F2ZFeLcsiLlLr89kqUl.u0kcmnykTv("No tables selected.", "IP_Assurance");
			return;
		}
		...
		try
		{
			...
			foreach (Table item in tables)
			{
				...
				{
					_G_o__5.tLZVTFA6XNG = CallSite<Func<CallSite, Type, object, object>>.Create(Binder.InvokeMember(CSharpBinderFlags.None, "ToString", null, typeof(GwPwjPKig3SC80BpOVZ), new CSharpArgumentInfo[2]
					{
				...
				{
					_G_o__5.Y9xVTPOsWB8 = CallSite<Func<CallSite, Type, object, object>>.Create(Binder.InvokeMember(CSharpBinderFlags.None, "ToString", null, typeof(GwPwjPKig3SC80BpOVZ), new CSharpArgumentInfo[2]
					{
						CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.UseCompileTimeType | CSharpArgumentInfoFlags.IsStaticType, null),
						CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null)
					}));
				...
				{
					_G_o__5.gXmVTq4CdC4 = CallSite<Func<CallSite, object, object>>.Create(Binder.GetMember(CSharpBinderFlags.None, "Value", typeof(GwPwjPKig3SC80BpOVZ), new CSharpArgumentInfo[1] { CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null) }));
				}
				if ((dynamic)target(y9xVTPOsWB, typeFromHandle, _G_o__5.gXmVTq4CdC4.Target(_G_o__5.gXmVTq4CdC4, worksheet.Cells[count2 + num - 1, count3])) == string.Empty)
				{
					...
					{
						_G_o__5.B4NVTW744L1 = CallSite<Func<CallSite, object, string, object>>.Create(Binder.SetMember(CSharpBinderFlags.None, "Value", typeof(GwPwjPKig3SC80BpOVZ), new CSharpArgumentInfo[2]
						{
							CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null),
	...
	public static void pSAKJ3DAt3()
	{
		try
		{
			...
			{
				F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(text2 + "  可直接粘贴", "IP_Assurance");
			}
	...
	private static Worksheet yekKEcMdXi(Workbook P_0, string P_1)
	{
		...
		{
			_G_o__10.yWZVTsqLW75 = CallSite<Func<CallSite, Sheets, object, object>>.Create(Binder.InvokeMember(CSharpBinderFlags.None, "Add", null, typeof(GwPwjPKig3SC80BpOVZ), new CSharpArgumentInfo[2]
			{
	...
	private static int YV1K26qhk7(Worksheet P_0)
	{
		...
		{
			_G_o__11.SUFVTov2Kyj = CallSite<Func<CallSite, object, object>>.Create(Binder.GetMember(CSharpBinderFlags.None, "Row", typeof(GwPwjPKig3SC80BpOVZ), new CSharpArgumentInfo[1] { CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null) }));
		}
		...
		{
			_G_o__11.MTDVTNjq95L = CallSite<Func<CallSite, object, object>>.Create(Binder.GetMember(CSharpBinderFlags.ResultIndexed, "End", typeof(GwPwjPKig3SC80BpOVZ), new CSharpArgumentInfo[1] { CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null) }));
		}
		...
		{
			_G_o__11.rmjVTpISiO9 = CallSite<Func<CallSite, Type, object, object>>.Create(Binder.InvokeMember(CSharpBinderFlags.None, "ToString", null, typeof(GwPwjPKig3SC80BpOVZ), new CSharpArgumentInfo[2]
			{
				CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.UseCompileTimeType | CSharpArgumentInfoFlags.IsStaticType, null),
				CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null)
			}));
		...
		{
			_G_o__11.jtZVTCjwVp8 = CallSite<Func<CallSite, object, object>>.Create(Binder.GetMember(CSharpBinderFlags.None, "Value", typeof(GwPwjPKig3SC80BpOVZ), new CSharpArgumentInfo[1] { CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null) }));
		}
