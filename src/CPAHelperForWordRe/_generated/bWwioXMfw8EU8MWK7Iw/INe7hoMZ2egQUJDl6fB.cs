...
namespace bWwioXMfw8EU8MWK7Iw;

internal static class INe7hoMZ2egQUJDl6fB
{
	...
	private static ftu1AgSpErBKqc6vp9f Cfg => ftu1AgSpErBKqc6vp9f.Current;
	...
	public static void lyrMMMAAtt(int P_0)
	{
		...
		if (paragraphs.OutlineLevel != (WdOutlineLevel)P_0)
		{
			...
			if (paragraphs.OutlineLevel != (WdOutlineLevel)P_0)
			{
				F2ZFeLcsiLlLr89kqUl.F9Ycoqv2I8("无法直接调整该段落的大纲级次，请检查是否已设置样式，请取消样式或手动调整样式级次", "IP_Assurance");
			}
		}
	}
	...
	public static void dR8MSOK1CL()
	{
		...
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo("你需要先选中一个批注", "IP_Assurance");
		}
	}
	...
	public static void kcGMtYHAg8()
	{
		try
		{
			...
			{
				F2ZFeLcsiLlLr89kqUl.F9Ycoqv2I8("未获取到 Excel/WPS 复制区域。请先在 Excel 中复制表格，或关闭任务管理器中的残留 Excel 后重试。", "IP_Assurance");
				return;
			}
			Selection selection = App.Selection;
			if (selection.Tables.Count == 0)
			{
				F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo("请将光标放在要粘贴的 Word 表格内", "IP_Assurance");
				return;
			}
		...
		{
			F2ZFeLcsiLlLr89kqUl.F9Ycoqv2I8("请检查任务管理器后台进程中是否有残留Excel " + ex.Message, "IP_Assurance");
		}
	}
	...
	private static bool dGUMmCaDyi(out int P_0, out int P_1)
	{
		...
		try
		{
			object obj = xcAKcictyYcoMl6ObA.X11yRCCGO()?.Selection;
			if (obj != null)
			{
				object arg = obj;
				if (_G_o__17.SLFVUj7dyZt == null)
				{
					_G_o__17.SLFVUj7dyZt = CallSite<Func<CallSite, Type, object, object>>.Create(Binder.InvokeMember(CSharpBinderFlags.None, "ToInt32", null, typeof(INe7hoMZ2egQUJDl6fB), new CSharpArgumentInfo[2]
					{
				...
				{
					_G_o__17.qehVU4CkmLe = CallSite<Func<CallSite, object, object>>.Create(Binder.GetMember(CSharpBinderFlags.None, "Count", typeof(INe7hoMZ2egQUJDl6fB), new CSharpArgumentInfo[1] { CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null) }));
				}
				...
				{
					_G_o__17.Od9VU2HR5gW = CallSite<Func<CallSite, object, object>>.Create(Binder.GetMember(CSharpBinderFlags.None, "Rows", typeof(INe7hoMZ2egQUJDl6fB), new CSharpArgumentInfo[1] { CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null) }));
				}
				...
				{
					_G_o__17.KfbVUMxDE0h = CallSite<Func<CallSite, Type, object, object>>.Create(Binder.InvokeMember(CSharpBinderFlags.None, "ToInt32", null, typeof(INe7hoMZ2egQUJDl6fB), new CSharpArgumentInfo[2]
					{
						CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.UseCompileTimeType | CSharpArgumentInfoFlags.IsStaticType, null),
						CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null)
					}));
				...
				{
					_G_o__17.EaiVUfwFhWg = CallSite<Func<CallSite, object, object>>.Create(Binder.GetMember(CSharpBinderFlags.None, "Count", typeof(INe7hoMZ2egQUJDl6fB), new CSharpArgumentInfo[1] { CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null) }));
				}
				...
				{
					_G_o__17.HOgVUZShiuk = CallSite<Func<CallSite, object, object>>.Create(Binder.GetMember(CSharpBinderFlags.None, "Columns", typeof(INe7hoMZ2egQUJDl6fB), new CSharpArgumentInfo[1] { CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null) }));
				}
	...
	{
		return (float)xXKxBLwR5fgTmDqdT91.NgZw6CkHuw(Cfg.KxPSXHwy4c(P_0, string.Empty), P_1);
	}
	...
	internal static void hn7MXTwgTj(Range P_0, bool P_1, Func<int, int, string, bool> P_2)
	{
		...
		try
		{
			...
			if (num2 > 0)
			{
				yR9thasHZ73xXm8eKwj.ujWsURly3F(string.Format("[FormatParagraph] Paragraph formatting partially failed; Total={0}; Failed={1}", count, num2), ex);
			}
		}
		finally
		{
	...
	public static void IlFMa7FUcE()
	{
		...
		F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(text, "IP_Assurance");
	}
