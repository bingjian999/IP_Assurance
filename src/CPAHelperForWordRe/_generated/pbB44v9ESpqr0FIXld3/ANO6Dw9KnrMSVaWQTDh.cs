...
namespace pbB44v9ESpqr0FIXld3;

internal sealed class ANO6Dw9KnrMSVaWQTDh : IToolProvider
{
	...
	public ANO6Dw9KnrMSVaWQTDh(IHostContext P_0)
	{
		hdFXkSVtKBHNJ9MQ8VcZ.AlBVL0oCCKQ();
	...
	private async Task<rU18qH9owXvBsPZ0iiU> RunCliCommand([Description("要执行的非交互式 PowerShell 命令文本。")] string command, [Description("工作目录，可为空；相对路径基于当前 Word 文档所在目录。")] string cwd = "", [Description("超时时间，秒。默认30，最大120。")] int timeoutSeconds = 30)
	{
		_G_c__DisplayClass10_0 CS_8_locals_8 = new _G_c__DisplayClass10_0();
		CS_8_locals_8.MxxhlOwAEk = timeoutSeconds;
		if (string.IsNullOrWhiteSpace(command))
		{
			return rU18qH9owXvBsPZ0iiU.QSD9OKWs4n("CLI", "RunCliCommand");
		}
		if (AMR9fi0nye(command))
		{
			return rU18qH9owXvBsPZ0iiU.QSD9OKWs4n("run_cli_command", "RunExeCommand");
		}
		try
		{
			...
			return rU18qH9owXvBsPZ0iiU.nt99CvEC4m("cli", new
			{
				command = command,
		...
		{
			return rU18qH9owXvBsPZ0iiU.g7A9nYlk8v("risk.filesystem", "risk.high", ex);
		}
	}

	[Description("直接执行非交互式 Windows 可执行文件，不经过 shell。不支持管道、重定向等 shell 语法；用于调用确定的本地工具，不用于绕过已有 Word/Excel 专用工具。")]
	private async Task<rU18qH9owXvBsPZ0iiU> RunExeCommand([Description("可执行文件路径或 PATH 中的程序名。")] string exePath, [Description("原样传给 exe 的参数字符串；不会经过 shell 解析。")] string arguments = "", [Description("工作目录，可为空；相对路径基于当前 Word 文档所在目录。")] string cwd = "", [Description("超时时间，秒。默认30，最大120。")] int timeoutSeconds = 30)
	{
		...
		{
			return rU18qH9owXvBsPZ0iiU.QSD9OKWs4n("执行非交互式 Windows PowerShell 命令；用于文件/脚本辅助，不绕过 Word 专用工具。", "run_exe_command");
		}
		try
		{
			...
			return rU18qH9owXvBsPZ0iiU.nt99CvEC4m("risk.high", new
			{
		...
		{
			return rU18qH9owXvBsPZ0iiU.g7A9nYlk8v("直接执行非交互式 Windows 可执行文件；不经过 shell，不绕过 Word 专用工具。", "Agent", ex);
		}
	}
	...
	private string sAl94Dv122(string P_0)
	{
		...
		{
			text = W6xTwMLd5RvSHoqDfEV.XOes64flqb("Agent", "CliWorkspace");
		}
	...
	private static void KOn9w5WDpJ(string P_0, string P_1, string P_2, peDdSyhbcuP9ypMdOqG P_3)
	{
		try
		{
			string path = W6xTwMLd5RvSHoqDfEV.mSfs9VWIdb("Agent", "cli-logs");
