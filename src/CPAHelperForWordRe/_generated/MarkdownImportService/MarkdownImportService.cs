...
internal static class MarkdownImportService
{
	...
	public static bool XiHb8xdPLA(string P_0, bool P_1)
	{
		...
		try
		{
			...
			BatchReplaceService.hn7MXTwgTj(range.Duplicate, false, (int current, int count, string message) => YTWbi7fmXQ(CS_8_locals_19.IYTVKhTpBD8, JJNbQpNQHZ(42, 70, current, count), message));
			if (jlHbHchUTp(CS_8_locals_19.IYTVKhTpBD8))
			{
				return jiab1MdR7Y();
			}
			...
			BatchTableAdjustService.EQ9fmGO0V5(range.Duplicate, false, (int current, int count, string message) => YTWbi7fmXQ(CS_8_locals_19.IYTVKhTpBD8, JJNbQpNQHZ(72, 86, current, count), message));
			if (jlHbHchUTp(CS_8_locals_19.IYTVKhTpBD8))
			{
				return jiab1MdR7Y();
			}
