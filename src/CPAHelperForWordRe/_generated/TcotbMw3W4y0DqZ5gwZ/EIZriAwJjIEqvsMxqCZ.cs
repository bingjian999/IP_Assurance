...
using yai2NVw5sMalsil5CUc;

namespace TcotbMw3W4y0DqZ5gwZ;

internal static class EIZriAwJjIEqvsMxqCZ
{
	[CompilerGenerated]
	...
	public static A6h5sUwLEOvWuLQLmiB TgvwKZvsfr(string P_0)
	{
		...
		return new A6h5sUwLEOvWuLQLmiB
		{
	...
	public static YjZ4lmw7an9JOgXHIuS gTQwEdX7kI(A6h5sUwLEOvWuLQLmiB P_0)
	{
		if (P_0 == null)
		{
		...
		YjZ4lmw7an9JOgXHIuS yjZ4lmw7an9JOgXHIuS = new YjZ4lmw7an9JOgXHIuS
		{
		...
		if (P_0.ConfigValues.Count > 0)
		{
			...
			ftu1AgSpErBKqc6vp9f.Current.TTVSewUqXZ(dictionary);
		}
		usdwt8HvqO(P_0.ParagraphPresetFiles, true, yjZ4lmw7an9JOgXHIuS);
		usdwt8HvqO(P_0.TablePresetFiles, false, yjZ4lmw7an9JOgXHIuS);
		yR9thasHZ73xXm8eKwj.swCsJ4IbrL("[ConfigMigration] Imported legacy Word table/paragraph config from: " + P_0.SourceConfigPath);
	...
	private static List<H2vq9wwGY8vBw2m8e3D> nIfwbYYaJt(string P_0, string P_1, string P_2)
	{
		List<H2vq9wwGY8vBw2m8e3D> list = new List<H2vq9wwGY8vBw2m8e3D>();
		if (string.IsNullOrWhiteSpace(P_0))
		{
			return list;
		}
		...
		try
		{
			...
			foreach (string text in files)
			{
				...
				if (!string.IsNullOrWhiteSpace(fileName))
				{
					...
					list.Add(new H2vq9wwGY8vBw2m8e3D
					{
						SourcePath = text,
						TargetPath = text2,
		...
		{
			yR9thasHZ73xXm8eKwj.z7Us3dJ6Cl("[ConfigMigration] Enumerate legacy preset files failed: " + ex.Message);
		}
		return list;
	}
	...
	{
		return W6xTwMLd5RvSHoqDfEV.mSfs9VWIdb("config", P_0);
	}

	private static string k5Www6bih8()
	{
		string mainConfigFilePath = W6xTwMLd5RvSHoqDfEV.MainConfigFilePath;
		if (!File.Exists(mainConfigFilePath))
		{
			return string.Empty;
		}
		string text = Path.Combine(W6xTwMLd5RvSHoqDfEV.ConfigDir, "backups");
		W6xTwMLd5RvSHoqDfEV.i24sVuaOic(text);
	...
	private static void usdwt8HvqO(IEnumerable<H2vq9wwGY8vBw2m8e3D> P_0, bool P_1, YjZ4lmw7an9JOgXHIuS P_2)
	{
		foreach (H2vq9wwGY8vBw2m8e3D item in P_0 ?? Enumerable.Empty<H2vq9wwGY8vBw2m8e3D>())
		{
			...
			try
			{
				...
				{
					W6xTwMLd5RvSHoqDfEV.i24sVuaOic(directoryName);
				}
