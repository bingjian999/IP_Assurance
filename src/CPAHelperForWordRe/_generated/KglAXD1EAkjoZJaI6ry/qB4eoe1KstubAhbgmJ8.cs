...
namespace KglAXD1EAkjoZJaI6ry;

internal sealed class qB4eoe1KstubAhbgmJ8
{
	[CompilerGenerated]
	private sealed class _G_c__DisplayClass2_0
	{
		...
		internal rU18qH9owXvBsPZ0iiU w4mdyDK6GA(Application app)
		{
			...
			try
			{
				...
				{
					_G_o__2.kxxdWPKgCB = CallSite<Func<CallSite, object, int, object>>.Create(Binder.SetMember(CSharpBinderFlags.None, "LockAspectRatio", typeof(qB4eoe1KstubAhbgmJ8), new CSharpArgumentInfo[2]
					{
				...
				BZOrlUu1R80X7V7qPHv.ySsuKsA6u8();
				return rU18qH9owXvBsPZ0iiU.nt99CvEC4m("HTML visual inserted into Word.", new
				{
	...
	private readonly x2u1CVJLYNVQcUFtkEy Cow1Z1ymNA;

	public qB4eoe1KstubAhbgmJ8(RkZt4ZuLjXTP5cAL48p P_0)
	{
		hdFXkSVtKBHNJ9MQ8VcZ.AlBVL0oCCKQ();
		Cow1Z1ymNA = new x2u1CVJLYNVQcUFtkEy(P_0);
	}

	public rU18qH9owXvBsPZ0iiU Uwp128MQKQ(string P_0, string P_1, int P_2, int P_3, double P_4, double P_5, string P_6, string P_7)
	{
		...
		try
		{
			...
			return Cow1Z1ymNA.MdXJlVhPku("create_html_visual", delegate(Application app)
			{
				...
				try
				{
					...
					if (_G_o__2.kxxdWPKgCB == null)
					{
						_G_o__2.kxxdWPKgCB = CallSite<Func<CallSite, object, int, object>>.Create(Binder.SetMember(CSharpBinderFlags.None, "[AI Tool][Word] Insert HTML visual failed.", typeof(qB4eoe1KstubAhbgmJ8), new CSharpArgumentInfo[2]
						{
							CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.None, null),
							CSharpArgumentInfo.Create(CSharpArgumentInfoFlags.UseCompileTimeType | CSharpArgumentInfoFlags.Constant, null)
						}));
					...
					BZOrlUu1R80X7V7qPHv.ySsuKsA6u8();
					return rU18qH9owXvBsPZ0iiU.nt99CvEC4m("HTML visual PNG file does not exist.", new
					{
						document = document.Name,
		...
		{
			yR9thasHZ73xXm8eKwj.ujWsURly3F("imagePath", ex);
			Exception baseException = ex.GetBaseException();
			return rU18qH9owXvBsPZ0iiU.g7A9nYlk8v("HTML visual display size must be greater than zero.", (baseException is ArgumentException) ? "create_html_visual" : "[AI Tool][Word] Insert HTML visual failed.", ex);
		}
	}
