...
namespace dpdiOwfLNrjZbpbZGek;

internal static class MTM8HqftP23tlVOKhjT
{
	...
	public static void w3YfsuDd2C()
	{
		...
		try
		{
			...
			{
				F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo("请将光标放在表格内", "IP_Assurance");
			}
			else
			{
	...
	public static void HUeflwYrZr()
	{
		...
		try
		{
			...
			else if (jrNMVJb8eb(activeDocument))
			{
				...
				if (!F2ZFeLcsiLlLr89kqUl.JWucG2ERAH(string.Format("将批量调整 {0} 个表格，是否确认？", count), "IP_Assurance"))
				{
					zIhMBFWLgR();
					arEM9085NB();
					return;
			...
			if (count2 == 0)
			{
				F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo("请选择一个或多个表格", "IP_Assurance");
				return;
			}
	...
	public static void HbTfxQrf6q()
	{
		try
		{
			...
			if (count < 2)
			{
				F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo("请选择大于一个表", "IP_Assurance");
				return;
			}
			...
			if (num != 2)
			{
				F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo("当前无法处理表头中有纵向合并单元格的表格", "IP_Assurance");
				return;
			}
		...
		{
			F2ZFeLcsiLlLr89kqUl.F9Ycoqv2I8(ex.Message, "IP_Assurance");
		}
	}
	...
	public static void PL3fzV7hld()
	{
		...
		try
		{
			...
			{
				F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo("文档已保护，此时不能选中多个表格！", "IP_Assurance");
				return;
			}
			zIhMBFWLgR();
			...
			EditorID = WdEditorType.wdEditorEveryone;
			activeDocument.DeleteAllEditableRanges(ref EditorID);
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(string.Format("已选中 {0} 个表格。", gfbMjFCyLI.Count), "IP_Assurance");
		}
		finally
		{
			try
	...
	{
		yR9thasHZ73xXm8eKwj.z7Us3dJ6Cl("[Tables] " + P_0 + " | " + P_1.Message);
	}

	static MTM8HqftP23tlVOKhjT()
	{
		hdFXkSVtKBHNJ9MQ8VcZ.AlBVL0oCCKQ();
