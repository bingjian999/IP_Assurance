...
namespace Vn2FNvXfBBVtqPcjIyB;

internal static class Xwl2EJXZ1waT9LlFYjb
{
	[CompilerGenerated]
	private sealed class _G_c__DisplayClass41_0
	{
		public Jk2boJKtvpW0C9HFNVs.B8Id9rVIwgTW2spgYNvs OQKVbeL2HSs;

		public ProgressWindow s7ZVbyORkkZ;

		public Jk2boJKtvpW0C9HFNVs.ffxcqxVIn1iOgk0nKLUY K2ZVbXqAMC3;
		...
		internal void kDJVb74DwvM()
		{
			wMSXdLtYmv(delegate
			{
				OQKVbeL2HSs = Jk2boJKtvpW0C9HFNVs.XVJ45smyDw((int current, int total, string message) => GQlFRxrmSC(s7ZVbyORkkZ, current, total, message));
			}, "数据同步");
		}
	...
	{
		public Jk2boJKtvpW0C9HFNVs.B8Id9rVIwgTW2spgYNvs BLYVba1rP6j;
		...
		internal void AGgVbhfKb0g()
		{
			try
			{
				BLYVba1rP6j = Jk2boJKtvpW0C9HFNVs.VuQ4c2ipxq();
			}
			catch (Exception ex)
			{
				yR9thasHZ73xXm8eKwj.ujWsURly3F("[ExcelSync] Context menu locate crashed", ex);
				BLYVba1rP6j = new Jk2boJKtvpW0C9HFNVs.B8Id9rVIwgTW2spgYNvs
				{
					Success = false,
	...
	public static void K5NXSH2mgl()
	{
		VvXX7RqLnd();
		IsLoaded = TOlXmIuJyF();
		if (IsLoaded && eSfxffslhXbaGAjFNv1.IsWps)
		{
			string text = bmJXClxTxp();
			if (!IsLoaded)
			{
				yR9thasHZ73xXm8eKwj.z7Us3dJ6Cl("[ExcelSync] WPS native context menu auto load skipped: " + text);
			}
			CtJXNX8dgh();
		}
		else
	...
	{
		if (!eSfxffslhXbaGAjFNv1.IsWps && IsLoaded)
		{
			return F2EXoDU3KJ();
		}
		return false;
	...
	private static bool TOlXmIuJyF()
	{
		try
		{
			string text = ftu1AgSpErBKqc6vp9f.Current.KxPSXHwy4c("Excel同步_右键菜单_自动加载", "false");
	...
	private static bool F2EXoDU3KJ()
	{
		try
		{
			Selection selection = eSfxffslhXbaGAjFNv1.WordApp?.Selection;
	...
	private static void LCjXG6voSH(bool P_0)
	{
		try
		{
			ftu1AgSpErBKqc6vp9f.Current.pE1SyAiPWc("Excel同步_右键菜单_自动加载", P_0 ? "false" : "true");
		}
		catch
		{
		}
	...
	private static void tr6Xh0jnAc(Application P_0, bool P_1)
	{
		...
		{
			yR9thasHZ73xXm8eKwj.z7Us3dJ6Cl("[ExcelSync] Restore NormalTemplate saved state failed: " + ex.Message);
		}
	}
		...
		TKkXkjis2w("同步数据", delegate
		{
			...
			try
			{
				HCDXxBpknD(delegate
				{
					wMSXdLtYmv(delegate
					{
						CS_8_locals_17.OQKVbeL2HSs = Jk2boJKtvpW0C9HFNVs.XVJ45smyDw((int current, int total, string message) => GQlFRxrmSC(CS_8_locals_17.s7ZVbyORkkZ, current, total, message));
					}, "[ExcelSync] WPS native context menu auto load skipped: ");
				});
			}
			catch (Exception ex)
			{
				yR9thasHZ73xXm8eKwj.ujWsURly3F("Locate", ex);
				CS_8_locals_17.OQKVbeL2HSs = new Jk2boJKtvpW0C9HFNVs.B8Id9rVIwgTW2spgYNvs
				{
					Success = false,
			...
			if (CS_8_locals_17.OQKVbeL2HSs != null)
			{
				...
				{
					yR9thasHZ73xXm8eKwj.swCsJ4IbrL("false" + CS_8_locals_17.OQKVbeL2HSs.Message);
					lRTy9Ic5uax0jm0RR1L.SeXce6fgLN(string.IsNullOrWhiteSpace(CS_8_locals_17.OQKVbeL2HSs.Message) ? "Excel同步_右键菜单_自动加载" : CS_8_locals_17.OQKVbeL2HSs.Message, "false");
				}
				else if (CS_8_locals_17.OQKVbeL2HSs.Cancelled)
				{
					lRTy9Ic5uax0jm0RR1L.Kn6cyKZe85(string.IsNullOrWhiteSpace(CS_8_locals_17.OQKVbeL2HSs.Message) ? "true" : CS_8_locals_17.OQKVbeL2HSs.Message, "无法获取 WPS 应用对象，右键菜单未加载。");
				}
				else if (!string.IsNullOrWhiteSpace(CS_8_locals_17.OQKVbeL2HSs.Message))
				{
					yR9thasHZ73xXm8eKwj.z7Us3dJ6Cl("无法获取 WPS CommandBars，右键菜单未加载。" + CS_8_locals_17.OQKVbeL2HSs.Message);
					lRTy9Ic5uax0jm0RR1L.IuZcXy6pki(CS_8_locals_17.OQKVbeL2HSs.Message, "同步数据");
				}
			}
		});
	}
	...
	private static void qLOXPSqh3g()
	{
		TKkXkjis2w("定位数据", delegate
		{
			...
			HCDXxBpknD(delegate
			{
				try
				{
					CS_8_locals_11.BLYVba1rP6j = Jk2boJKtvpW0C9HFNVs.VuQ4c2ipxq();
				}
				catch (Exception ex)
				{
					yR9thasHZ73xXm8eKwj.ujWsURly3F("CPAHelper.ExcelSync.SyncData", ex);
					CS_8_locals_11.BLYVba1rP6j = new Jk2boJKtvpW0C9HFNVs.B8Id9rVIwgTW2spgYNvs
					{
						Success = false,
			...
			{
				ygcXvh8pOP();
				lRTy9Ic5uax0jm0RR1L.SeXce6fgLN(string.IsNullOrWhiteSpace(CS_8_locals_11.BLYVba1rP6j.Message) ? "CPAHelper.ExcelSync.LocateData" : CS_8_locals_11.BLYVba1rP6j.Message, "已加载右键菜单。");
			}
			else if (!string.IsNullOrWhiteSpace(CS_8_locals_11.BLYVba1rP6j.Message))
			{
				GxjXAL3yeg(CS_8_locals_11.BLYVba1rP6j.Message);
				yR9thasHZ73xXm8eKwj.z7Us3dJ6Cl("加载右键菜单失败：没有找到可挂载的 WPS 表格右键菜单。" + CS_8_locals_11.BLYVba1rP6j.Message);
				lRTy9Ic5uax0jm0RR1L.Kn6cyKZe85(CS_8_locals_11.BLYVba1rP6j.Message, "同步数据");
			}
			else
			{
				ygcXvh8pOP();
	...
	private static string lwdX09nj1e()
	{
		try
		{
			Jk2boJKtvpW0C9HFNVs.gMo5J5VI9LOS3nqRiEJZ gMo5J5VI9LOS3nqRiEJZ = Jk2boJKtvpW0C9HFNVs.inT4f2XO4S(false, false);
	...
	private static void wMSXdLtYmv(Action P_0, string P_1)
	{
		...
		try
		{
			...
			{
				yR9thasHZ73xXm8eKwj.z7Us3dJ6Cl("[ExcelSync] Context menu StartCustomRecord failed: " + ex.Message);
			}
			P_0();
		}
		finally
		{
			if (flag)
			{
				...
				{
					yR9thasHZ73xXm8eKwj.z7Us3dJ6Cl("[ExcelSync] Context menu EndCustomRecord failed: " + ex2.Message);
				}
			}
		}
	}
