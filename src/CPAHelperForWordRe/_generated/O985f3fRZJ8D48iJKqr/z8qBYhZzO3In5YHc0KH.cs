...
namespace O985f3fRZJ8D48iJKqr;

internal static class z8qBYhZzO3In5YHc0KH
{
	...
	public static void a1VfDCrrNS()
	{
		...
		{
			F2ZFeLcsiLlLr89kqUl.F9Ycoqv2I8("插入页码失败：" + ex.Message, "IP_Assurance");
		}
		finally
		{
	...
	public static void ykFfgO7l4C()
	{
		...
		{
			App.ScreenUpdating = false;
			if (openFileDialog.ShowDialog(rA7neb5TDVvwyWyxwua.KJy58rGLXb()) != DialogResult.OK)
			{
				return;
			}
			flag = true;
		...
		if (flag)
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(string.Format("批量转为 PDF 完成：{0} / {1}", num, num2), "IP_Assurance");
		}
	}
	...
	public static void kB3fQeM4Kl()
	{
		...
		{
			F2ZFeLcsiLlLr89kqUl.u0kcmnykTv("请先保存当前文档", "IP_Assurance");
			return;
		}
		if (!activeDocument.Saved)
		{
			if (!F2ZFeLcsiLlLr89kqUl.JWucG2ERAH("当前文档有未保存的修改。按页拆分需要先保存当前文档，是否保存并继续？", "IP_Assurance"))
			{
				return;
			}
			activeDocument.Save();
		...
		if (num <= 0)
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo("当前文档没有可拆分的页面。", "IP_Assurance");
			return;
		}
		...
		if (flag)
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(string.Format("按页拆分已取消，已生成 {0} / {1} 个文档。", num5, num3), "IP_Assurance");
		}
		else
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(string.Format("按页拆分完成，共生成 {0} 个文档。", num5), "IP_Assurance");
		}
	}
	...
	public static void Wq4f1QfrnV()
	{
		...
		if (!activeDocument.Saved)
		{
			if (!F2ZFeLcsiLlLr89kqUl.JWucG2ERAH("当前文档有未保存的修改。按指定页拆分需要先保存当前文档，是否保存并继续？", "IP_Assurance"))
			{
				return;
			}
			activeDocument.Save();
		...
		IReadOnlyList<RsQuUTnPQGmBy8VcAkC> readOnlyList = imKfSm7rVB(num);
		...
		try
		{
			...
			{
				RsQuUTnPQGmBy8VcAkC rsQuUTnPQGmBy8VcAkC = readOnlyList[num3];
		...
		if (flag)
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(string.Format("按指定页拆分已取消，已生成 {0} / {1} 个文档。", num2, count), "IP_Assurance");
		}
		else
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(string.Format("按指定页拆分完成，共生成 {0} 个文档。", num2), "IP_Assurance");
		}
	}
	...
	public static void vFrfrDlytO()
	{
		...
		if (!activeDocument.Saved)
		{
			if (!F2ZFeLcsiLlLr89kqUl.JWucG2ERAH("当前文档有未保存的修改。按分节符拆分需要先保存当前文档，是否保存并继续？", "IP_Assurance"))
			{
				return;
			}
			activeDocument.Save();
		...
		if (count <= 1)
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo("未发现可用于拆分的分节符。", "IP_Assurance");
			return;
		}
		int num = 0;
		bool flag = false;
		...
		if (flag)
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(string.Format("按分节符拆分已取消，已生成 {0} / {1} 个文档。", num, count), "IP_Assurance");
		}
		else
		{
			F2ZFeLcsiLlLr89kqUl.Ay3cNuEgJo(string.Format("按分节符拆分完成，共生成 {0} 个文档。", num), "IP_Assurance");
		}
	}
	...
	private static string BiIf3e4mrZ(string P_0, RsQuUTnPQGmBy8VcAkC P_1)
	{
		string directoryName = Path.GetDirectoryName(P_0);
		string fileNameWithoutExtension = Path.GetFileNameWithoutExtension(P_0);
		string text = Path.GetExtension(P_0);
	...
	private static int LUVfb1Q91y(int P_0)
	{
		try
		{
			...
			return (rA7neb5TDVvwyWyxwua.NJs5HCHQjv(splitPagesWindow) == true) ? splitPagesWindow.PagesPerDocument : 0;
		}
		catch (Exception ex)
		{
			F2ZFeLcsiLlLr89kqUl.F9Ycoqv2I8("打开拆分页数窗口失败：" + ex.Message, "IP_Assurance");
			return 0;
		}
	}

	private static IReadOnlyList<RsQuUTnPQGmBy8VcAkC> imKfSm7rVB(int P_0)
	{
		try
		{
			SplitPageRangesWindow splitPageRangesWindow = new SplitPageRangesWindow(P_0);
			return (rA7neb5TDVvwyWyxwua.NJs5HCHQjv(splitPageRangesWindow) == true) ? splitPageRangesWindow.PageRanges : null;
		}
		catch (Exception ex)
		{
			F2ZFeLcsiLlLr89kqUl.F9Ycoqv2I8("打开指定页拆分窗口失败：" + ex.Message, "IP_Assurance");
			return null;
		}
	}
