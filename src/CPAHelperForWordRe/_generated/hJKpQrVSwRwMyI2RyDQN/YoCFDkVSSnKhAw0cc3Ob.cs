...
namespace hJKpQrVSwRwMyI2RyDQN;

internal class YoCFDkVSSnKhAw0cc3Ob
{
	...
	static YoCFDkVSSnKhAw0cc3Ob()
	{
		MrsVw319ZOP = false;
		auHVwUyvsY2 = typeof(YoCFDkVSSnKhAw0cc3Ob).Assembly;
		...
		try
		{
			...
			if (UCNVwjjwmBX == null)
			{
				lock (lL2VwY3JnIT)
				{
					...
					BinaryReader binaryReader = new BinaryReader(typeof(YoCFDkVSSnKhAw0cc3Ob).Assembly.GetManifestResourceStream("2BWb04irCgC5cG0Wfb.3XyKq7HoOCWibfsyqt"));
			...
			for (int m = 0; m < fields.Length; m++)
			{
				try
				{
					...
					num12 &= 0x3FFFFFFF;
					MethodInfo methodInfo = (MethodInfo)typeof(YoCFDkVSSnKhAw0cc3Ob).Module.ResolveMethod(num12, typeFromHandle.GetGenericArguments(), new Type[0]);
	...
	private static void Uo9VShsM3hK(Stream P_0, int P_1)
	{
		...
		while (true)
		{
			...
			while (true)
			{
				...
				case 78:
				case 125:
					new YoCFDkVSSnKhAw0cc3Ob().SLUVSGoY6ab(array5, array3, array6);
					num2 = 301;
					continue;
				case 233:
					array[11] = 205;
